...
if __name__ == "__main__":
    ...
    print("Generating the (simulation) Evolution Density Figure -- evolution.py")

    fig = plt.figure(figsize=(7.8, 7.8*(21./30.)))
    ...
    for plot_ax, data in zip(axes, snapshots):
        plot_ax, img = plot_single(plot_ax,
                                   np.log10(sim_data[data].sd_map),
                                   r"$t={}$ Gyr".format(data*0.02),
                                   cmap,
                                   vmin,
                                   vmax,
                                   extent)
    ...
    cb.set_ticks(np.arange(-1, 3.5, 0.5))
    ticklabels = [r"$\leftarrow$ -1    ", "-0.5", "0", "0.5", "1", "1.5", "2.0", "2.5", r"   3.0 $\rightarrow$"]
